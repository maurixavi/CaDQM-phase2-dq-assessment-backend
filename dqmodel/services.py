...
class DQExecutionResultService:
    ...
    @staticmethod
    @transaction.atomic(using='metadata_db')
    def save_execution_result(dq_model_id, applied_method, result_value, execution_details, result_type=None, execution_id=None):
        """
        Versión que:
    ...
    def save_execution_resultOk(dq_model_id, applied_method, result_value, execution_details, result_type=None):
        """
        Versión que:
        1. Para resultados únicos: guarda solo el valor en dq_value
        2. Para resultados múltiples: guarda en dq_value la estructura completa con rows
